...
from .access import batch_course_access, get_course_access, student_has_full_access
...
def catalog(request):
    ...
    levels = (
        Level.objects.filter(is_published=True)
        .select_related("language", "instructor")
        .annotate(published_lesson_count=Count("modules__lessons", filter=Q(modules__lessons__is_published=True)))
    ...
    page = paginate_queryset(request, levels, per_page=DEFAULT_PAGE_SIZE)
    page_levels = list(page.object_list)

    access_map = batch_course_access(request.user, page_levels)
    wishlist_ids = set()
    if request.user.is_authenticated and getattr(request.user, "is_student", False):
        wishlist_ids = set(
            Wishlist.objects.filter(student=request.user, level_id__in=[l.id for l in page_levels]).values_list(
                "level_id", flat=True
            )
        )

    course_cards = [
        {"level": level, "access": access_map[level.id], "in_wishlist": level.id in wishlist_ids}
...
def course_detail(request, language_slug, level_slug):
    ...
    level = get_object_or_404(
        Level.objects.select_related("language", "instructor")
        .prefetch_related("modules__lessons", "modules__quizzes", "modules__assignments", "quizzes")
        .annotate(published_lesson_count=Count("modules__lessons", filter=Q(modules__lessons__is_published=True))),
    ...
    if not request.user.is_authenticated:
        cache.set(cache_key, level.pk, 300)
